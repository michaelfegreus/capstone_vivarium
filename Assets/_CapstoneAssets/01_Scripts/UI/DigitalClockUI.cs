...
public class DigitalClockUI : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ClockManager.OnMinuteTick += UpdateClockValues;
    }
    void OnDisable()
    {
        ClockManager.OnMinuteTick -= UpdateClockValues;
    ...
    private void Start()
    {
        ...
        if (ClockManager.Instance.inGameTime.TimeMet(new GameTime(12, 0))){
            AM = false;
        }
        else
        {
    ...
    void UpdateClockValues()
    {
        ...
        int hour = ClockManager.Instance.inGameTime.hour;
        int minute = ClockManager.Instance.inGameTime.minute;
