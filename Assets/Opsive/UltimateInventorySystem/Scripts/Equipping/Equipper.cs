        ...
        public virtual void UpdateBones()
        {
            ...
            for (int i = 0; i < m_Bones.Length; i++) {
                ...
                if (m_BonesDictionary.ContainsKey(bone.name)) {
                    Debug.LogWarning($"Bones in the '{m_MainRootNode.name}' bone hierarchy must all have unique names. The bone name '{bone.name}' is used multiple times.", m_MainRootNode);
                    continue;
                }
