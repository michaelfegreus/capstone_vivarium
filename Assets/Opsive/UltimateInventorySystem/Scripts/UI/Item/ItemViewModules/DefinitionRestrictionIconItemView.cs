...
namespace Opsive.UltimateInventorySystem.UI.Item.ItemViewModules
{
    ...
    public class DefinitionRestrictionIconItemView : ItemViewModule
    {
        ...
        {
            var categoryRestriction = View.ViewSlot.gameObject.GetComponent<ItemViewSlotDefinitionRestriction>();
            if (categoryRestriction == null) {
                m_Icon.sprite = m_DefaultSprite;
                return;
            }
            
            if (categoryRestriction.ItemDefinition.TryGetAttributeValue<Sprite>(m_IconAttributeName, out var icon)) {
                m_Icon.sprite = icon;
            } else {
                m_Icon.sprite = m_DefaultSprite;
            }
