...
namespace Fungus
{
    ...
    public enum ClickMode
    {
        ...
        ClickOnButton,
        /// <summary> Custom Vivarium -- press one of the Vivarium GAME_input_manager buttons to advance. </summary>
        VivariumInput
    }
    ...
    public class DialogInput : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            case ClickMode.Disabled:
                break;
            case ClickMode.VivariumInput:
                if (Input.GetMouseButtonDown(0) || Input.GetKeyDown(GAME_manager.Instance.inputManager.selectionButton1) || Input.GetKeyDown(GAME_manager.Instance.inputManager.selectionButton2))
                {
                    SetNextLineFlag();
                }
                break;
...
/*
case ClickMode.VivariumInput:
                // Might want to add others, like to have an A button as well as B button advance the text for example.
                if (Input.GetKeyDown(GAME_manager.Instance.inputManager.selectionButton1) || Input.GetKeyDown(GAME_manager.Instance.inputManager.selectionButton2))
                {
                    SetNextLineFlag();
                }
                // Also include ClickOnDialogue option
                if (dialogClickedFlag)
                {
                    SetNextLineFlag();
dialogClickedFlag = false;
