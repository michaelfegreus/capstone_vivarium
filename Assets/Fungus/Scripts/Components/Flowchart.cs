...
namespace Fungus
{
    ...
    {
        //-----------------------------------------------------------------------------------------
        // Vivarium modification code:
        //-----------------------------------------------------------------------------------------

        public delegate void GlobalVariableChange();
        public static event GlobalVariableChange OnGlobalVariableChange;

        public virtual void TriggerGlobalVariableChange()
        {
            if (OnGlobalVariableChange != null)
            {
            ...
            for (int i = 0; i < variables.Count; i++)
            {
                ...
                if (v != null && v.Key == key)
                {
                    ...
                    if (variable != null)
                    {
                        ...
                        // Vivarium -- Run global variable change event delegate.
                        TriggerGlobalVariableChange();
                        return;
                    }
                }
            }
