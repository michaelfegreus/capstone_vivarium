...
namespace Fungus
{
    ...
    public static class TextTagParser
    {
        ...
        {
            return "" +
                "\t NOTE: To use SuperTextMesh text tags, use {w=0}<b>{w=60} format to prevent showing tag briefly.\n" +
                "\n" +
                "\tColor [<c> / <c=name> / <c=hexcode> / <c=DefaultColorName>]" +
                "\tChange color/gradient/texture of text. Hex code is without the # symbol, and supports alpha.\n" +
                "\tBold[< b >] Set text to be bold.Can stack with italic.\n" +
                "\tItalic [<i>] Set text to be italic. Can stack with bold.\n" +
                "\tWave [<w> / <w=name>] Use a wave from the Text Data file.\n" +
                "\tJitter [<j> / <j=name>] use a jitter fromt he Text Data file.\n" +
                "\tAlignment Tags [<a=alignment>] These will set text alignment, mid-string. See documentation.\n" +
                "\tLine Break [<br> / \\n] Good for inserting line breaks on single-line text entry fields.\n" +
                "\tRelative Size [<s=float>] Change size of text, relative to the default size.\n" +
                "\tSize [<size=float>] Set the exact size of text.\n" +
                "\tEvent [<e=string>] Will invoke a custom event when the text mesh reads it.\n" +
                "\tRepeating Event [<e2=string>] Repeat for each letter until cancelled with </e2>.\n" +
                "\tFont [<f=name>] Use a font, defined in Text Data.\n" +
                "\t{b} Bold Text {/b} (Unity Text only.)\n" + 
                "\t{i} Italic Text {/i} (Unity Text only)\n" +
                "\t{color=red} Color Text (color){/color} (Unity Text Only)\n" +
                "\t{size=30} Text size {/size} (Unity Text Only)\n" +
                "\n" +
